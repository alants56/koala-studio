...
server.registerTool('koala_create_automation', {
  title: '创建 Koala 自动化', description: '创建一条自动化规则。重要：创建 trigger=指定时间 的启用任务时，必须同时传 schedule 和 actionType。actionType=claude_prompt 会启动独立 Claude Code 会话执行 instruction，且必须提供绝对 projectPath；actionType=create_high_priority_todo 会在工作台创建重点待办。仅在 triggerDetail 中写“计划于某时”不会触发执行。',
  inputSchema: { name: z.string().min(1).max(120), description: z.string().max(500).optional(), trigger: z.string().min(1).max(120), triggerDetail: z.string().max(120).optional(), action: z.string().min(1).max(120), actionDetail: z.string().max(120).optional(), scope: z.string().min(1).max(120), enabled: z.boolean().optional(), schedule: scheduleSchema.optional(), actionType: z.enum(['feature_brief', 'claude_prompt', 'create_high_priority_todo']).optional(), projectPath: z.string().min(1).max(4096).optional(), instruction: z.string().min(1).max(4000).optional().describe('actionType=claude_prompt 时必填，由 Claude Code 在 projectPath 中自主完成。') },
...
server.registerTool('koala_update_automation', {
  ...
  inputSchema: { id: z.string().min(1), name: z.string().min(1).max(120).optional(), description: z.string().max(500).optional(), trigger: z.string().min(1).max(120).optional(), triggerDetail: z.string().max(120).optional(), action: z.string().min(1).max(120).optional(), actionDetail: z.string().max(120).optional(), scope: z.string().min(1).max(120).optional(), schedule: scheduleSchema.nullable().optional(), actionType: z.enum(['feature_brief', 'claude_prompt', 'create_high_priority_todo']).optional(), projectPath: z.string().min(1).max(4096).nullable().optional(), instruction: z.string().min(1).max(4000).nullable().optional() },
