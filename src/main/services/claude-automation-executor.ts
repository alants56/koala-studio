...
/** Runs one scheduled instruction in an isolated Claude Code ACP session. */
export async function executeClaudeInstruction(
  ...
  if (!cwd) throw new Error('Claude Code 定时任务缺少项目文件夹。')
  if (!instruction) throw new Error('Claude Code 定时任务缺少自定义指令。')
  ...
  const handleUpdate = (notification: SessionNotification): void => {
    ...
    if (update.sessionUpdate === 'agent_message_chunk' && update.content.type === 'text') {
      ...
      log(`Claude Code 调用工具：${update.title}`)
  ...
  try {
    ...
    const response = await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeout = setTimeout(() => reject(new Error('Claude Code 执行超过 30 分钟，任务已终止。')), EXECUTION_TIMEOUT_MS)
      })
    ])
    if (response.stopReason !== 'end_turn') throw new Error(`Claude Code 未正常完成任务（${response.stopReason}）。`)

    const content = responseParts.join('').trim() || 'Claude Code 已完成指令，但没有返回文本结果。'
    log('Claude Code 已返回最终结果', 'success')
    return {
      summary: 'Claude Code 已完成自定义指令',
      detail: '完整回复已保存到本次运行结果。',
      output: { title: `${automation.name} · Claude Code 结果`, content, format: 'markdown' }
    }
  } catch (error) {
